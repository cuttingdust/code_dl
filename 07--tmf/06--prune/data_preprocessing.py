...
def collate_fn(batch_data):
    ...
    # 可以不要下面的内容。是因为上面是对句子一条条处理的，因此token_type_ids中的值全都0。
...
if __name__ == "__main__":
    # 数据预处理只需要Tokenizer，不需要在这里额外加载一份BERT模型。
    # 使用验证集取一个批次，检查三个张量的形状。
    for input_ids, attention_mask, labels in build_dataloader(
        config.dev_datapath, False
    ):
