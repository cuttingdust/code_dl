...
# app_streamlit.py 位于 code_dl/07--tmf/05--distill 目录中。
...
def main() -> None:
    """绘制新闻分类页面，并调用Flask后端中的蒸馏学生模型完成预测。"""
    # 前端只负责收集输入和显示结果，不会直接加载PyTorch模型。
    # 真正的Tokenizer编码和BiLSTM学生模型推理都在Flask后端中完成。

    # 1- 创建蜂窝头条新闻分类页面
    st.title("蜂窝头条新闻分类")
    st.write("使用知识蒸馏后的 BiLSTM 学生模型预测新闻类别")

    # 2- 获取用户输入的新闻标题
    title = st.text_input("请输入新闻标题")

    # 3- 单击按钮后，把新闻标题包装成JSON并发送给Flask预测接口。
    # 请求流程：Streamlit -> Flask /predict_api -> student_predict_service -> 学生模型。
    if st.button("提交"):
...
def run() -> None:
    ...
    if st.runtime.exists():
        # 使用streamlit run app_streamlit.py或
        # python -m streamlit run app_streamlit.py启动时，Streamlit运行时已经存在，
