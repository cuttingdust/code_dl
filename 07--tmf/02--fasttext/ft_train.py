...
def char_manual_train():
    ...
    print(f"子词 --> {model.get_subwords("ab")}")

    # 4.4- 词的维度
    print(f"词的维度 --> {model.get_dimension()}")


@MPoint(append_message="自动调整超参数")
def char_auto_train():
    # 1- 模型训练
    """
    参数解释：
...
def word_manual_train():
    ...
    # 4- 子词：查看minn和maxn的作用
    print(f"子词 --> {model.get_subwords("人工智能")}")
